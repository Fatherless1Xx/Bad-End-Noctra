...
/datum/tgui
	...
	var/interface
	/// Optional UI key for per-source window tracking.
	var/ui_key = "main"
 ...
 */
/datum/tgui/New(mob/user, datum/src_object, interface, title, ui_x, ui_y, datum/tgui/master_ui, datum/ui_state/state)
	var/list/arg_list = args
	var/ui_key = null
	// Old signature: (user, src_object, ui_key, interface, title, ui_x, ui_y, master_ui, state)
	if(length(arg_list) >= 9)
		ui_key = arg_list[3]
		interface = arg_list[4]
		title = arg_list[5]
		ui_x = arg_list[6]
		ui_y = arg_list[7]
		master_ui = arg_list[8]
		state = arg_list[9]
	// Old signature without master_ui/state: (user, src_object, ui_key, interface, title, ui_x, ui_y)
	else if(length(arg_list) >= 7 && istext(ui_x))
		ui_key = interface
		interface = title
		title = ui_x
		ui_x = ui_y
	...
	src.ui_key = ui_key || "main"
	src.window_key = "[REF(src_object)]-[src.ui_key]"
	src.interface = interface
	if(title)
		src.title = title
	src.state = state || src_object.ui_state(user)
	// Deprecated
	if(ui_x && ui_y)
