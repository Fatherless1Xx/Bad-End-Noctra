 ...
 * * full_heal/admin_revive - Legacy named args. If provided, they are mapped to heal flags.
 *
 */
/mob/living/proc/revive(full_heal_flags = NONE, excess_healing = 0, force_grab_ghost = FALSE, full_heal = null, admin_revive = null)
	// Legacy positional support: revive(TRUE, TRUE) => full_heal/admin_revive.
	if(isnull(full_heal) && isnull(admin_revive))
		if(isnum(full_heal_flags) && isnum(excess_healing) && (full_heal_flags == 0 || full_heal_flags == 1) && (excess_healing == 0 || excess_healing == 1) && !force_grab_ghost)
			full_heal = full_heal_flags
			admin_revive = excess_healing
			full_heal_flags = NONE
			excess_healing = 0

	// Legacy named args support.
	if(!isnull(full_heal) || !isnull(admin_revive))
		if(admin_revive)
			if(full_heal_flags)
				full_heal_flags |= HEAL_ADMIN
			else
				full_heal_flags = ADMIN_HEAL_ALL
		else if(full_heal && !full_heal_flags)
			full_heal_flags = HEAL_ALL
