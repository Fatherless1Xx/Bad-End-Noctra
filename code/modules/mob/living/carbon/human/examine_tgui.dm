...
/datum/examine_panel/ui_data(mob/user)
	...
	if(ishuman(holder))
		...
		if(!holder.wear_armor && !holder.wear_shirt)
			is_naked = TRUE
		obscured = (!isobserver(user)) && ((holder_human.wear_mask && (holder_human.wear_mask.flags_inv & HIDEFACE)) || (holder_human.head && (holder_human.head.flags_inv & HIDEFACE)))
		...
		if(!obscured)
			...
			img_gallery = holder.img_gallery || list()
			nsfw_img_gallery = holder.nsfw_img_gallery || list()
		if(!headshot)
		...
		img_gallery = pref.img_gallery || list()
		nsfw_img_gallery = pref.nsfw_img_gallery || list()
