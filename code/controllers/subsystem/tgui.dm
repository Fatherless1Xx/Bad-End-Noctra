...
/datum/controller/subsystem/tgui/proc/try_update_ui(
		...
		datum/tgui/ui = null,
		force_open = FALSE,
		datum/tgui/master_ui = null,
		datum/ui_state/state = null)
	// Backwards-compat: allow passing ui in the ui_key slot.
	if(istype(ui_key, /datum/tgui) && isnull(ui))
		ui = ui_key
		ui_key = null
	if(force_open)
		return null
 ...
 * return datum/tgui The found UI.
 */
/datum/controller/subsystem/tgui/proc/get_open_ui(mob/user, datum/src_object, ui_key = null)
	// No UIs opened for this src_object
	if(!LAZYLEN(src_object?.open_uis))
		return null
	for(var/datum/tgui/ui in src_object.open_uis)
		// Make sure we have the right user
		if(ui.user == user && (!ui_key || ui.ui_key == ui_key))
			return ui
	return null
