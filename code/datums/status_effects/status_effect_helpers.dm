...
/mob/proc/apply_status_effect(datum/status_effect/new_effect, duration_override, ...)
	...
	if(initial(new_effect.status_type) != STATUS_EFFECT_MULTIPLE)
		for(var/datum/status_effect/existing_effect as anything in L.status_effects)
 ...
 */
/mob/proc/remove_status_effect(datum/status_effect/removed_effect, ...)
	if(!isliving(src))
		return FALSE
...
/// Returns the blindness status effect if the mob is blinded from the passed sources.
/mob/proc/is_blind_from(sources)
	return has_status_effect_from_source(/datum/status_effect/grouped/blindness, sources)
